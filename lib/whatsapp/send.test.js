...
test('builds an approved image-header template card', () => {
  const payload = buildWhatsAppTemplateCardPayload(card, 'sor7ed_protocol_card')

  assert.equal(payload.type, 'template')
  assert.equal(payload.template.name, 'sor7ed_protocol_card')
  assert.equal(payload.template.components[0].parameters[0].image.link, card.imageUrl)
  assert.equal(payload.template.components[2].parameters[0].text, card.slug)
})
