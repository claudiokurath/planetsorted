    ...
    >
      ...
      <div className={`flex min-w-0 flex-1 flex-col justify-between gap-3 p-4 sm:p-6 ${compact ? 'sm:min-h-52' : `${coverImage ? 'sm:min-h-64' : 'sm:min-h-72'} sm:gap-5 lg:p-7`}`}>
        <div className="flex flex-col flex-1">
          ...
          <h3
            className={`font-bebas ${compact ? 'text-[2.15rem] sm:text-5xl' : 'text-[2.15rem] sm:text-6xl lg:text-7xl'} mt-4 line-clamp-3 font-extralight uppercase leading-[0.9] text-white transition-colors group-hover:text-[#C6A052] sm:mt-8`}
          >
            {title}
          </h3>
          {summary ? (
