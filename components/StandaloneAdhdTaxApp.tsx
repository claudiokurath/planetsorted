    ...
    <div className="min-h-screen bg-black text-white">
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-10 sm:px-6 lg:px-8">
        <header className="rounded-3xl border border-neutral-800 bg-neutral-950 p-8 shadow-2xl">
          <p className="text-xs font-bold uppercase tracking-[0.35em] text-[#F5C518]">PLANET SOR7ED LAB — Wealth</p>
          <h1
            className="font-bebas mt-3 text-5xl uppercase leading-[0.95] tracking-tight sm:text-6xl md:text-7xl"
          >
            ADHD Tax Calculator
          </h1>
          <p className="mt-4 max-w-2xl text-lg font-medium text-neutral-400">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="rounded-3xl border border-neutral-800 bg-neutral-950 p-6 shadow-2xl">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {[
                ...
                <label key={field.label} className="block text-sm text-neutral-300">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-neutral-800 bg-black px-4 py-3 text-white outline-none ring-0 focus:border-[#F5C518]"
                  />
                </label>
              ))}
            </div>
          ...
          <section className="rounded-3xl border border-[#F5C518]/30 bg-[#F5C518]/10 p-6 shadow-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#F7D154]">Estimated annual leak</p>
            <div className="font-bebas mt-3 text-5xl font-black sm:text-6xl">£{result.yearlyTotal.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div>
            <p className="mt-2 text-neutral-300">That’s about £{result.monthlyTotal.toLocaleString(undefined, { maximumFractionDigits: 2 })} per month.</p>

            <div className="mt-6 space-y-3">
              {Object.entries(result.breakdown).map(([key, value]) => (
                <div key={key} className="flex items-center justify-between rounded-2xl border border-neutral-800 bg-neutral-950/80 px-4 py-3 text-sm">
                  <span className="capitalize text-neutral-400">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                  <span className="font-semibold text-white">£{value.toFixed(2)}</span>
                </div>
              ))}
            </div>

            <div className="mt-6 rounded-2xl border border-neutral-800 bg-neutral-950/80 p-4">
              <h3 className="text-lg font-semibold">Quick reset plan</h3>
              <ul className="mt-3 space-y-2 text-sm text-neutral-300">
                {result.actionPlan.map((step) => (
                  <li key={step} className="flex gap-2">
                    <span className="text-[#F7D154]">•</span>
                    <span>{step}</span>
                  </li>
                ))}
              </ul>
