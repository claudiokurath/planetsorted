...
/**
 * A single row in the "From the blog" strip. Paper theme.
...
export function BlogRow({ href, date, kicker, title, body, meta }: BlogRowProps) {
  return (
    <Link
      href={href}
      className="group grid gap-4 border-t border-[rgba(10,10,10,0.10)] py-6 md:grid-cols-[auto_1fr_auto] md:items-baseline md:gap-10 md:py-8"
    >
      <span className="themed-idx md:pt-1">{date}</span>
      <div>
        <span className="themed-idx block">{kicker}</span>
        <h3 className="mt-2 font-bebas text-3xl uppercase leading-tight tracking-normal md:text-4xl">
          {title}
        </h3>
