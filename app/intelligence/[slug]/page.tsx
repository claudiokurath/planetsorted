...
export default async function ArticlePage({ params }: Props) {
  ...
  const { data: rawProtocol } = await supabase
    .from('protocols')
    // 'protocol' deliberately excluded — WhatsApp-only content, never the webpage
  ...
  // Website shows the story only — never the WhatsApp step-by-step script
      ...
      <article className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {protocol.cover_image && (
          <div className="relative aspect-video w-full mb-8 overflow-hidden rounded-2xl shadow-xl border border-[#E8E0D5]">
            <Image src={protocol.cover_image} alt={`${protocol.title} — Planet Sorted illustration`} fill className="object-cover" priority unoptimized />
          </div>
        )}
        ...
        <h1 className="mb-4 text-4xl sm:text-5xl md:text-6xl font-black uppercase leading-tight text-[#1A1A1A]" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
          {protocol.title}
        </h1>
        ...
        {audioUrl && (
          <div className="my-8 rounded-2xl border border-[#E8E0D5] bg-white p-6 shadow-sm space-y-3 max-w-3xl">
            ...
            <a href={waAudioUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm font-bold text-[#C0392B] underline hover:text-red-800 transition-colors pt-1">
        ...
        {protocol.excerpt && (
          <div className="mb-8 p-6 bg-white/60 border-l-4 border-[#C0392B] rounded-r-2xl shadow-sm max-w-3xl">
            <p className="text-xl sm:text-2xl leading-relaxed font-semibold text-[#1A1A1A]">{protocol.excerpt}</p>
          </div>
        )}
        ...
        <div className="my-12 rounded-2xl bg-[#1A1A1A] p-8 sm:p-10 text-white shadow-2xl space-y-6 max-w-3xl">
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <a href={waClickUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#C0392B] px-8 py-4 text-base sm:text-lg font-bold uppercase tracking-wider text-white hover:bg-red-700 transition-colors shadow-lg">
