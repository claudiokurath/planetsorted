...
- **Notes (0.4.22):** Repo hygiene + build health (commit `363be4f`). Two raw standalone-tool templates that had been dropped into `app/(standalone)/` with no `page.tsx` (and therefore no route) — `friction-wallet` and `rsd-assessment` (the latter's folder name carried a trailing space) — moved to `raw-standalone-templates/` alongside the others; a redundant 185 MB nested clone of this repo and a stray `rsd-assessment  copy/` removed. Two generated XLSX registers committed by accident in `3f7a62b` untracked from the tree and `/outputs/` + `/tmp/` added to `.gitignore` (**note:** the XLSX blobs remain in history at `3f7a62b` — a `filter-repo` purge is still outstanding pending a PII check). `.dockerignore` widened (was shipping ~1 GB of `raw-standalone-templates/` in the build context). ESLint now ignores `tmp/`/`outputs/`; `package.json` set to `"type": "module"` + `engines.node >= 22`; CI lint step made blocking now the pre-existing error backlog is clear. No product/behaviour changes.
